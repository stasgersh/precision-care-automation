...
if (-not $ACTION -or -not $API_URL -or -not $JWT_ACCESS_TOKEN -or -not $FOLDER) {
    Write-Host "Usage: .\representation-labels.ps1 -ACTION <ACTION> -API_URL <API_URL> -JWT_ACCESS_TOKEN <JWT_ACCESS_TOKEN> -FOLDER <FOLDER>"
    exit 1
}
...
foreach ($File in $JSONFiles) {
    ...
    # Extract labels array from the JSON file
    $Content = Get-Content -Path $File.FullName | ConvertFrom-Json
    if (-not $Content.labels) {
        Write-Host "No labels found in $($File.Name). Skipping..."
        continue
    }
    ...
    foreach ($Label in $Content.labels) {
        ...
        if ($ACTION -eq "create" -or $ACTION -eq "update") {
            ...
            } -Body ($Label | ConvertTo-Json -Depth 10) -ErrorAction SilentlyContinue -StatusCodeVariable StatusCode
