...
if (-not $ACTION -or -not $API_URL -or -not $JWT_ACCESS_TOKEN -or -not $FOLDER) {
    Write-Host "Usage: .\fulfillment-valueset.ps1 -ACTION <ACTION> -API_URL <API_URL> -JWT_ACCESS_TOKEN <JWT_ACCESS_TOKEN> -FOLDER <FOLDER>"
    exit 1
}
...
foreach ($File in $JSONFiles) {
    ...
    if ($ACTION -eq "update" -or $ACTION -eq "delete") {
        ...
        $URL = "${API_URL}value-set/$ID"
    } else {
        $URL = "${API_URL}value-set"
    }
